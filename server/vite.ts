...
function applySecurityHeaders(res: express.Response) {
  ...
  res.setHeader(
    "Permissions-Policy",
    'geolocation=(), microphone=(), camera=(), payment=(self "https://js.stripe.com" "https://m.stripe.com")',
  );
  ...
  const replitSources =
    isDevelopment || isStaging
      ? " https://replit.com https://*.replit.com https://*.replit.dev https://*.kirk.replit.dev https://cdn.jsdelivr.net"
      : "";
  const localhostSources = isDevelopment
    ? " http://localhost:5173 http://localhost:5000 http://127.0.0.1:5173 http://127.0.0.1:5000 https://*.kirk.replit.dev:5173"
    : "";
  const websocketSources =
    isDevelopment || isStaging
      ? " ws://localhost:5173 wss://localhost:5173 wss://*.replit.dev wss://*.kirk.replit.dev"
      : "";
